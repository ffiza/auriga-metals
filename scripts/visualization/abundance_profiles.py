...
def plot_fit_stats(sample: list, config: dict):
    ...
    for i, simulation in enumerate(sample):
        with open(f"results/{simulation}/"
            ...
            lreg = json.load(f)
            ax.errorbar(
                lreg["slope"], 0 - 0.02 * i, xerr=lreg["stderr"], color="gray",
                markeredgecolor="white", capsize=2, capthick=1,
                markeredgewidth=0.75, marker='o', markersize=4,
    ...
    ax.add_patch(r)
    median_str = str(np.abs(np.round(np.mean(sample_slopes), 4)))
    ax.text(x=-0.012, y=-0.555, size=6.0, color="gray",
